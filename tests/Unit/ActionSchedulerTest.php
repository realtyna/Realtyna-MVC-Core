...
class ActionSchedulerTest extends \WP_UnitTestCase {
	...
	public function test_if_single_action_is_running_in_1_minute() {
		add_action( 'test_hook_for_1_min', [ $this, 'changeTitle' ] );
		$title = 'Hello World!';

		update_option( 'blogname', 'Test 2' );

		$timestamp = strtotime( '+1 minutes' );
		$hook      = 'test_hook_for_1_min';
		$args      = array( $title);

		as_schedule_single_action( $timestamp, $hook, $args );
		$this->assertNotEquals('Test 2', $title);
		sleep(70);
		ActionScheduler::runner()->run();
		$this->assertEquals($title, get_option('blogname'));
	}
